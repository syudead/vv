...
1. **グループは作り直せる索引の表（`folder_groups`・`folder_group_members`）に持ち、作り直しは
   ...
   取引の中、例外の変更の取引の中、起動時に索引の版が古いとき、の4か所である**
...
7. **一覧の絞り込みはメンバー単位、視聴状態の絞り込みと並べ替えは項目単位で行う。グループの集計値
   ...
   動画の `id`、グループは最初のメンバーの `id` で、項目どうしで重ならない。
...
12. **自動再生は画面の中で行う。予告が終わった時点で次のメンバーを `GET /api/videos/{id}` で確かめ、
    無ければ再生終了の表示にする。** 次のメンバーは開いたときの関連動画の応答で決まり、途中で
    取り直さない（Edge Case「再生中にグループが変わる」）。移るときは前後のつまみと同じく
    `state.from` を引き継ぐので、閉じる操作は開く前の一覧へ戻る（要件 30）。
    - 却下: 予告の間に SSE で次のメンバーの消失を見張る案。予告の終わりに1回確かめれば、見える結果
      （自動で移らず再生終了の表示）は同じである。
...
例外の変更・起動時の版）、`FolderGroupStore` の例外の設定と解除。ARCHITECTURE.md と
`internal/app/scans.go` の「スキャンの後に全体を読まない」の記述を、索引を読むことを含めた形に直す
...
**Acceptance**: store と httpapi のテストが、受け入れ条件 1・2・10（13 の並び順とシャッフルで項目が
...
**Scope**: ライブラリを `GET /api/library` に切り替え、`useVideos`・`listSnapshot` の項目を
`LibraryItem` にそろえ（Structural Decisions 13）、グループのカード、グループのカードの選択（全メンバー）、
「すべて選択」の `GET /api/library/ids`、件数、グループの項目の取り直しを足す。使われなくなる
`GET /api/videos/ids` を消す。`ui-design.md` の該当する節に従う。
...
ルートの検索結果が今のまま1本ずつ出ること（受け入れ条件 18）をテストが確かめる。`task check` が通る。
