...
| サムネイル | 小さなフォルダの絵柄（`FolderArt` の `size="row"`、下見なし）。下端の進捗は `inProgress` のとき `watchedCount / videoCount` |
| 題名 | グループ名（リンク、`/videos/{openVideoId}`）。その下に `text-xs text-fg-muted` の1行で「12 本」 |
