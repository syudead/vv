...
「視覚的階層」「情報密度」）。フォルダカード（011）のタブ付きの外形は使わない。ライブラリでは
グループは「動画と同格の1件」であり、フォルダ画面の「開いて中を見るもの」ではないからである。

- **サムネイル**: `cover`（残っているメンバーのうち並びで最初のもの）のサムネイル。無いときは
  `VideoCard` と同じ代わりの表示。hover プレビューは `cover` の `previewUrl` で、動画のカードと
  同じ規則（400ms、同時に1件）で流す。
- **サムネイルの上の目印**: 右下の時間の面（`bg-navbar/85`・`text-[11px]`・`tabular-nums`）の中を、
  左から lucide `Folder`（`size-3`、`aria-hidden`）→ 本数 → 合計の長さの順にする。公開の印
  （016「Card」）を出す場所と同じ面で、面を増やさない。本数の書き方は次のとおり。
  ...
  合計の長さ（`durationMs`）が無ければ長さを省き、目印と本数だけにする。本数の「3 / 12」は
  長さと同じ大きさ・同じ面の従の情報で、題名より目立たせない（UI品質「タイポグラフィ」）。
- **左上**: 選択のチェック（今と同じ位置）。フォルダの目印をここに置く案は採らない。チェックと
  重なる。
...
- 追加日時・大きさ・「フォルダ」の文字は出さない。カードに足すのは目印と本数だけで、高さは
...
| サムネイル | `cover` のサムネイル。下端の進捗は `inProgress` のとき `watchedCount / videoCount` |
...
- どちらも、応答を受けたらライブラリの控えを捨てる（Structural Decisions 10）。フォルダ画面の
  一覧は1本ずつのままなので読み直さない（`FolderSummary` の `grouping` だけ応答で差し替える）。
  次にライブラリを開くと、再スキャンなしにカードが変わる（受け入れ条件 3・4・5）。
...
- ライブラリの格子: グループのカード（未視聴・視聴中「3 / 12 本」・視聴済み）とふつうの
...
- **視覚的階層**
  - 格子でグループのカードとふつうのカードを見比べたとき、目が行く順はどちらもサムネイル →
    題名 → タグである。フォルダの目印と本数は長さと同じ小ささで面の中にあり、題名より先に
    目に入らない。目印が accent の色や大きなアイコンなら不合格。
...
- **タイポグラフィ**
  - グループ名はカードの題名と同じ書体と大きさで、「3 / 12 本」は長さと同じ大きさで桁がそろう。
...
- **改善と感じるべき人の見方**（Issue「UI品質とアクセシビリティ」）: `series/ep01..12` が
  ライブラリで1枚のカードになり、視聴中の本数が読め、押すと `ep04`（途中まで見たもの）が続きから
...
3. 再生画面で Tab を Group line の引き金へ進めて Enter → ↓ で「まとめを解除」→ Enter →
