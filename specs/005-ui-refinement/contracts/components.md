...
| C2 | Logo | `layout/Logo.tsx` | **する**（`/` へのリンク・`h1`） | 持つ | FR-003・[R-508](../research.md) |
...
**機能する対話要素は 7 つ**（C2・C3 の 1 行・C6 の 1 つ・C8・C10・C11・C12・C15）で、
これは 004 の時点で利用者が行えた操作と同じ集合である（C2 のロゴは一覧 `/` への
リンクで、行き先が増えていない）。**増えていないことが FR-013・SC-004 の要求**である。

## 2. 4 状態（FR-007 / FR-008）

C2・C3（`live`）・C6（`live`）・C8・C10・C11・C12・C15 は次の 4 つを視覚的に区別する。
...
対象は [data-model.md 2.](../data-model.md) の `kind: "inert"` の 10 行と、C7 の 3 つ
（設定・フィルタ・表示切替）である。
