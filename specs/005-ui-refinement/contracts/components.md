...
**どれが機能するかは spec が決めている。**本書が推測する余地は無く、次の 3 か所が
そろって同じ線を引いている。

| spec の箇所 | C3「すべての動画」/ C6「動画」 | C2 Logo |
| --- | --- | --- |
| 3.「表示のみの要素」 | **機能する**と明記 | 表にそもそも無い |
| 5.「既存機能との対応」 | 接続先（既存の一覧）を明記 | 行が無い |
| 2.「操作できるコンポーネントの状態」 | 4 状態を持つものとして名指し | 名指しされていない |
| 2.「作る UI コンポーネント」の役割 | — | 「製品アイコン + 製品名の 2 段」（表示物） |

C3 と C6 は spec が機能すると定めた 2 つなので `live` にする（FR-005 はこの表のとおりに
分けることを求めている）。C2 は 3 か所すべてで機能する側に置かれていないので、
リンクにしない。

リンクにしない実利も同じ向きである。ロゴを `/` へのリンクにすると、フォーカス可能な
要素と `Tab` の停止位置が 1 つ増え、検索中に押せば絞り込みが消える — 行き先が既存でも
**利用者にできることは 1 つ増える**。素の一覧へ戻る道は C3 の `live` な 1 行が既に
持っているので、増やす必要も無い。
...
| 段階 | 地 / 文字 | 使う場面 |
| --- | --- | --- |
| 情報 | `surface-raised` / `body` | 蔵書が空、該当なし |
...
知らせは**動画プレーヤーを画面外へ押し出さない高さ**にする（spec US4-6）。
