...
004 の表に**次の 3 組を足す**。`web/src/theme/tokens.test.ts` の `pairs` にも同じ 3 行を
足すこと。**表に無い組は検査されない。**
...
| `--size-tile-dense` / `-standard` / `-relaxed` と格子の式 | そのまま。本機能は列の式に触れない。**ただし列の「数」は変わる** — 幅 640 以上では器からサイドバーの 240px が引かれるため。新しい期待値は [layout.md 1.「器の幅と列数」](./layout.md)にあり、実装の回に [004 の contracts/design-tokens.md](../../004-library-ui/contracts/design-tokens.md) 3. の表をその値で更新する |
