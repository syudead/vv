...
- 列の部分は左右 `px-6`、上 `pt-6`、下 `pb-16`。列の間は `gap-6`。
...
- プレイヤーは `rounded-lg`、`overflow-hidden`、`bg-navbar`。幅は 16:9 の動画が画面の高さに
  ...
  `max-w-[calc((100dvh-12.25rem)*16/9)]` とし、見出しの帯の高さ（`3.25rem`）の分を差し引く。
...
- `lg` 以上ではページを画面の高さに留め（`h-dvh`、ページ自体はスクロールしない）、左の列と
  ...
  スクロールする。
- 列ごとのスクロールバーは `scrollbar-on-hover`（`web/src/index.css`）で細くし、ふだんは透明にして、
  その列にマウスが乗っているかフォーカスがあるときだけ見せる。列ごとに常に線が並ぶと騒がしい
  からである。スクロール自体はいつでもできる。
- 状態:
