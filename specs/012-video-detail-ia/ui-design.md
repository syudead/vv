...
- 右列は `lg` で `w-80`、`xl` 以上で `w-96`。関連動画の見出しの行（右端に ×）と、関連動画の
  縦の並びを置く。
- 右列の上端はプレイヤーの上端と揃える。
- プレイヤーは `rounded-lg`、`overflow-hidden`、`bg-navbar`、動画の縦横比（解像度が分からない
  間は 16:9、極端な比率は 9:16〜21:9 に丸める）で、高さが画面に収まるように幅を制限する。
  既存の `max-w-[calc((100dvh-12rem)*16/9)]` と同じ考え方で、上部バーが無くなった分だけ余裕を
  広げる。縦長の動画でも操作バーが収まるよう、幅は 18rem（列がそれより狭ければ列の幅）を下限とする。
