...
- 左から次の順に置く。
  ...
  - 前の動画（lucide `SkipBack`。同じフォルダの自然順の前の動画へ移る）
  - 再生/一時停止
  - 次の動画（lucide `SkipForward`。同じフォルダの自然順の次の動画へ移る）
  - 音量（ミュート）
  - 現在時刻 / 長さ
  - 右寄せの空き
  ...
  - ピクチャーインピクチャー
  - 全画面
- 秒数送り（10 秒戻る/進む）のボタンは置かない。秒数送りは ←/→ キーで行う。
- 前後の動画は、再生中の有無にかかわらず常に置く。関連動画の応答の `prevId`・`nextId` が
  無いとき（フォルダの先頭・末尾、関連動画の読み込み中・失敗）は押せなくする（`disabled`、
  不透明度 0.4）。再生中か見終えた後に移ったときは、移った先で再生を始める。止まっている間に
  移ったときは始めない。戻り先（`state.from`）は持ち越す。
