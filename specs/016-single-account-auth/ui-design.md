...
- **再生画面（`/videos/:id`）**: 所有者では題名の下の題名とタグのまとまりの直下に、公開の
  切り替えを足す。ゲストでは再生位置・タグ・ファイルの場所・読み取りのやり直しを出さない。
...
- 409 `account_already_configured` は、失敗の行に「アカウントは既に設定されています」を出し、
  ...
  （`Link`、`text-link underline underline-offset-4`）を出す。フォーカスはこのリンクへ移す。
...
- 「ログアウト」は `NavLink` ではなく `button` で、押すと `POST /api/auth/logout` を送り、
  ...
  出し、項目を戻す。
- 「ログイン」は `NavLink` で、`/login?next=<今の URL>` へ移る。再生画面から来た `next` も
  同じく今の URL である（サーバーが確かめて `redirectTo` にする）。
...
| 再生画面 | タグの並び（入力を含む）、ファイルの場所、LAST PLAYED の項目、読み取り失敗の「もう一度読み取る」「ファイルを開く」と `probeError` の枠 | 題名の下は区切り線と属性の一列だけになる。属性は 6 項目（LAST PLAYED を抜く）。再生は先頭から始める（`progress` が無い） |
...
- 題名とタグのまとまりの直下、区切り線の上に、公開の切り替えを1行で置く。左寄せで、
  `h-8`。まとまりとの間は `gap-2`（題名とタグの間と同じ）、区切り線との間は `gap-5`
