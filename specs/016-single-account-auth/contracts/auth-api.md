...
| 同じ送信元の失敗が直近 5 分で 5 回に達している | `429` `{ "code": "login_throttled", … }` と `Retry-After`（秒） |
...
- 送信元は、信頼するプロキシを経た場合はその転送ヘッダーから求めたクライアントの
  ...
  送信元とみなす。
- `redirectTo` は `next` を `domain` の規則で確かめた値である。`/` で始まり `//` と `/\` で
  始まらないパスと問い合わせ文字列だけを通し、`/login`・`/api/` 以下・解釈できない値・
  省略は `/` にする。画面はこの値へ遷移するだけで、自分では判定しない。
