...
2. **画面の認証は SPA のゲートで行い、`index.html` と `/assets/*` は認証なしで配る。**
   ...
   `authenticated` 以外なら `/login?next=<今の場所>` へ置き換える。`/login` は
...
6. **ログインの試行制限は、`internal/app` のメモリ上で送信元ごとに数える。** 直近 5 分の
   失敗が 5 回に達した送信元は、最も古い失敗が 5 分を過ぎるまで 429 にする。429 は
   数えず、成功で消す。IPv6 は /64 を1つの送信元とする。記録は古いものから捨て、
   送信元の数に上限を置く（要件 11）。Argon2id の照合は同時に2つまでにする。
...
`https://evil.example/`・`/login`・`/api/videos`・空を `/` にする。ユーザー名の規則は空・
...
照合を呼ばずに制限の誤りになり、時計を 5 分進めると正しい資格情報で成功する。別の
...
`r.TLS` をそのまま送信元と HTTPS の判定に使う。`cmd/mdm` で組み立て、起動時に期限切れの
セッションを消す。e2e は `run-e2e.mjs` がコマンドでアカウントを作り、Playwright の
セットアップが `POST /api/auth/login` でログインした状態を全テストに渡す。
...
終わる。同じデータディレクトリで組み立て直したサーバーでも、期限内の Cookie は通る。
90 日を過ぎたセッションは 401 になる。セッションの確認で DB が失敗すると 500 になり、
保護対象を返さない。3通りの誤りのログインが同じ状態・本文になる。別オリジンの
...
`MDM_TRUSTED_PROXIES`）と Caddy の設定例を書き、`compose.yaml` で環境変数を渡せるようにする。
...
`/login?next=…` へ、`setupRequired` で案内へ、`authenticated` で `/login` から戻り先へ
移ることが確かめられる。`web/e2e/auth.e2e.ts` で、未認証で `/`・`/settings`・`/videos/{id}` を
