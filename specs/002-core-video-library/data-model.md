...
001 が作った最小スキーマ（`videos` と `videos_fts`）を拡張し、利用者データと
ジョブの表を足す。列の設計方針は
...
001 で作った `id` / `path` / `title` / `size_bytes` / `mtime` / `added_at` に次を足す。
...
- 検証規則:
  - `path` は絶対パスで、NFC 正規化済み、`MDM_MEDIA_DIR` の内側であること
  - `title` は拡張子を除いたファイル名（NFC）。空になる場合はファイル名をそのまま使う
...
`title` と `path` の FTS5 外部コンテンツ表（trigram）。同期トリガも 001 のまま使う。
