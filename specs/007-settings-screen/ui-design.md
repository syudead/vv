...
4. FolderPlus icon付きprimary action「フォルダを追加」
5. 登録済みfolder rows、またはempty state
...
- **360px**: AppShellは既存drawerを使い、「設定」選択後に閉じる。見出し、説明、追加actionを
  縦に並べる。folder rowはpathを上、actionsを下端右へ置き、長い単語にも`overflow-wrap`を適用する。
  pickerはfull-screen dialogにする。
- **768px**: 既存rail sidebarを維持する。section heading/descriptionと追加actionを同じheader rowに
  置ける場合だけ横並びにし、folder rowはpathとactionsを横並びにする。pickerは中央modalにする。
...
- picker、変更確認、削除確認の全actionへTab/Shift+Tabで到達できる。破壊的確認の初期focusを
  danger actionへ置かない。
- `prefers-reduced-motion`では既存base ruleに従い、dialogとToastの動きだけを抑える。
...
1. 1280px: 複数folderを持つ設定画面。見出し、説明、追加、rows、row actionsの階層を確認する。
...
- **Visual hierarchy**: 「設定」→「メディアフォルダ」→現在値→対象row actionsの順に見え、補足文や
  danger actionが通常時の追加actionより強く見えない。
