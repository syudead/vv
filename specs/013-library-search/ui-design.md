...
| 最近再生した順 | 再生日時 | `History` | 降順 | 前に再生した順／最近再生した順 |
| ランダム | ランダム | `Shuffle` | — | — |
...
- 例は押しても検索欄に入らない。入れる案は、押しただけで今の検索語が置き換わる事故を
  招くので採らない。
- Esc は、ポップオーバーが開いているときはポップオーバーだけを閉じ、フォーカスを
  手引きのボタンに戻す。検索語は消さない（受け入れ条件 16）。検索欄にフォーカスがある
  ときの Esc（検索語を消して抜ける）は今のままである。ポップオーバーはモーダルにせず、
  中に操作を持たない。Tab で次の絞り込みボタンへ進むと閉じる。
...
- 説明の位置に、効いている条件を `ui/Chip`（neutral）で1行ずつ折り返して並べる。
...
- 検索中（検索語が空でないとき）は、パンくず帯の現在地の直後に `text-fg-muted` で
  ...
  代わりに「すべてのフォルダを検索中」とする。狭い幅でパンくずを省略するときも、この
  文字は省略しない（先に祖先の段が「…」になる）。パンくず帯は sticky なので、スクロール
  しても範囲が見える。
...
- 読み上げ名: 検索欄は上の3通り。手引きのボタンは「検索の書き方」で、ポップオーバーは
  ...
  並べ替えのメニューボタンは「並べ替え: 〈種類〉」。向きと並べ直すは上のとおり。
...
支援技術の確認では、アクセシビリティツリー（Playwright の `accessibility.snapshot` か
Chromium の DevTools）で、検索欄の3通りの名前、向きの名前が押すたびに変わること、
手引きのポップオーバーの名前、置き場所を含むカードの名前、要約行と隠した状態の行が
`polite` の live region であることを確かめる。実際の読み上げソフトでの確認は library-ui.md 5 の
