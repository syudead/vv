...
- **余白のリズム**: フォルダ画面の検索結果で、パンくず帯 → 件数 → 格子の間隔と
  カードの間隔が、ライブラリのツールバー → 件数 → 格子と同じに見える。
...
- **検索結果と通常表示の区別**（Issue「フォルダ画面の検索結果が、通常のフォルダ表示と
  ...
  パンくず帯の「内を検索中」と検索欄の検索語で、通常表示と取り違えない。
...
件数の行が `polite` の live region であることは、`ariaSnapshot` に出ないので
