...
- [X] T038 [quickstart.md](./quickstart.md) の S1〜S10 を順に実行し、すべて期待どおりであることを確認する — S1〜S10 すべて期待どおり。コンテナ経路も確認した（ポート公開、`HEALTHCHECK`、`/media` の読み取り専用バインド、`docker volume rm vv_data` からの復帰、終了コード `0`）。唯一の制約は、この開発環境では `dl-cdn.alpinelinux.org` が許可リストに無く最終段の `apk add` が通らないため、実行時の確認では `ffmpeg`／`ffprobe` をスタブに差し替えた点。本物が入ることは CI の Docker ジョブで確認済み。内訳は [docs/exec-plans/completed/001-initial-setup.md](../../docs/exec-plans/completed/001-initial-setup.md) の「受け入れ検証の内訳」
