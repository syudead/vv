...
- **ライブラリ（`/`）の格子表示のカード**: 題名の下にタグの行を足す（要件 3）。ライブラリの
  リスト表示の行（`VideoRow`）にはタグの行は無い（Plan の Structural Decisions 10）。
  フォルダ画面のカードにも #308 で同じタグの行が付き、押すとそのタグで絞ったライブラリ
  （`/?tag=<id>`）へ移る。カードの題名の下にあった「追加日時 · ファイルサイズ ·
  コーデック」の行は、#308 でどの画面のカードからも無くなった。
- **ライブラリの本文の先頭**: タグで絞り込んでいるときだけ、要約行の上に「絞り込み中の
  タグ」の行を置く。ツールバー（トップバーの中）には足さない。
- **再生画面（`/videos/:id`）**: 左の列の、題名のすぐ下（区切り線と属性の一列の上）に
  タグの並びを足す。
...
- 今のバー（`h-11`・`bg-elevated`・`shadow-elevated`）の中を、左から「N 件を選択中」→
  ...
  - 016 で「タグを外す」と縦線の間に「公開」メニューが加わった
    （[016 の ui-design.md「Selection bar」](../016-single-account-auth/ui-design.md#selection-bar)）。
...
- 360px（`sm` 未満）では1行に収まらないので、バーを2段にする。上の段に「N 件を選択中」・
  ...
  - 016 からは下の段が「タグを付ける」「タグを外す」「公開」の3つになり、3つが収まらない
    幅では「公開」だけが次の段の右端に回る（016 の ui-design.md「Selection bar」）。
...
- 再生画面: 題名の下のタグの並び、候補の一覧を開いた状態、「「anime」を作成」の行、101 文字の理由。
...
- **視覚的階層**
  ...
  - 一覧で accent の色を持つのは、絞り込み中のタグの行と絞り込みボタンだけで、カードの
    タグは中立の色のままである。
  - 再生画面で目が行く順は、プレイヤー → 題名 → タグ → 関連動画で、タグは題名の従として
    題名に付いて見える。タグの並びが題名より目立ったら不合格である。
...
- **余白のリズム**
  - カードの題名とタグの行の間隔が、今の題名とメタ情報の行の間隔と同じに見える。
    タグどうしの間隔がそろっている。
  - 再生画面で、題名とタグの間は狭く、タグと区切り線・区切り線と属性・属性とファイルの場所の
    間は今の同じ段階に見える。題名とタグが1つのまとまりに読める。
- **タイポグラフィ**
