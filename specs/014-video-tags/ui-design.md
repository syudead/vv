...
- 各チップは、`Tag` のアイコン・名前・`X` のアイコンを持つ1つのボタンで、どこを押しても
  ...
  チップ全体が1つの操作しか持たないので、的を小さくする理由が無い。hover で文字を
  `text-accent-hover` にし、押せることを示す。面の色は変えない。
...
- `h-6`・`text-xs`・`bg-elevated`・`text-fg`。名前の部分と × の部分の2つのボタンに分け、
  間に `bg-border-strong` の縦線（`w-px h-3.5`）を置く。
  - 名前の部分: 押すと `/?tag=<id>` のライブラリ一覧を開く。読み上げ名は「〈名〉で絞り込む」。
...
- キー: ↓・↑ で行を移り、端で止まる。Enter は選んでいる行を確定する。行を選んでいない
  ...
  シノニムがあればそのタグ、無ければ新しいタグになる。Tab は一覧を閉じて次へ進み、確定
  しない。
- 名前の検証（要件 4）: 入力のたびに確かめ、作れない名前のときは入力の下に `text-xs
  text-danger` で理由を1行出し、Enter を受けない。理由は「名前を入力してください」
  （空・空白だけ）、「改行やタブは使えません」、「100 文字以内にしてください（今 101 文字）」
  である。入力に `maxLength` は付けない。付けると 101 文字目が黙って捨てられ、理由を
...
- 押すと、バーの上に `ui/Popover`（`side="top"`）を開き、中に「Combobox」を置く。
...
- 「シノニム」を押すと、`ModalFrame`（Plan の Structural Decisions 9 で `web/src/ui/` へ
  ...
  - 上に、今のシノニムを `Chip`（neutral）に × を足した形で並べる。× の読み上げ名は
...
- 登録しようとした名前が既存のタグの名前のとき（要件 6「シノニム登録」）は、同じ窓の中を
  ...
  - 文言: 「「anime」は 10 本の動画に付いているタグです。「Anime」に統合すると、その 10 本に
    「Anime」が付き、「anime」は「Anime」のシノニムになります。「anime」はタグの一覧から
    消えます。」本数は `GET /api/tags` の `videoCount`（受け入れ条件 17）。
...
- 色の組: 文字に使う組のうち、`fg`・`fg-muted` on `elevated`（チップ・候補）と、`danger` on
  `bg`（再生画面と管理画面の理由と失敗の行）は、`tokens.test.ts` の `pairs` に既に入っている。
  選択バーのポップオーバーの失敗の行は `danger` on `elevated` になり、これは `pairs` に無い
  （計算では約 5.9 で 4.5 を満たす）。選択バーの一括操作を足す実装 PR で、この組を `pairs` に
  足す。絞り込み中のタグの `link` on `accent-soft` は、今の `Chip` の accent と同じ組で、
...
2. カードのチェックで1本を選び、Tab で選択バーの「タグを付ける」→ Enter → 入力に名前を
   打って ↓ で候補を選び Enter → トーストが出る → 「タグを外す」→ ↓ と Enter で外す。
3. 再生画面で Tab を「タグを追加」まで進め、打って ↓・↑ で候補を選び Enter → 入力に
   フォーカスが残る → Esc で一覧を閉じ、もう一度 Esc で入力が空になり、画面は閉じない →
