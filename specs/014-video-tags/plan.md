...
15. **Web のタグ絞り込みの条件はライブラリが持ち、共有の `web/src/videoList/` の
    ...
    - 一致なし（0 件）の状態は、013 の今の契約（PR #262）のとおり条件のチップや
      「条件を解除」を持たない。タグは `conditionLabels` に足さず、一致なしの上に出る
      「絞り込み中のタグ」の行と、絞り込みメニューの「条件を解除」（タグを含めてすべて
      外す）だけでタグの絞り込みを見せる・外せるようにする。
