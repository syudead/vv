...
- 「絞り込み（n 件適用中）」の n は、選んだタグ1つを1件として足す（要件 5）。
- #195 の「条件を解除」は `tag` も消す。#195 の `ui-design.md` は、絞り込みの数と
  「条件を解除」を出す条件を視聴状態・再生可否（と検索語）だけで書いている。タグを
  それに含める見た目の改め方は、この feature の design 工程で決める。
- 管理画面からタグを選んで移るときは、`/?tag=<id>` を開く。ほかの条件は付けない
  （受け入れ条件 16）。並べ替えは #195 の規則どおり端末に保存したものになる。
