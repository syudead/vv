...
- フローティング進捗は `AppShell` の右下に固定し、ライブラリ、フォルダ、設定画面で同じ
  表示を使う。再生画面は独立したシアターモードなので表示しない。
...
再生への導線より弱く見せる。右下に `fixed` で置き、本文の layout と scroll へ参加しない。
...
- 右端と下端からの余白は 360px でも安全領域を残す。既存 Toast と同時に出る場合は Toast より
  上へ逃がし、どちらの文言も読める位置関係にする。
...
- Popover は trigger の左上方向へ開くことを基本にし、Radix の collision handling で画面外へ
  はみ出さない。360px 幅では本文を長時間覆わない横幅へ収め、件数は折り返さず表のように読める。
...
- 360px: インジケーターは右下に収まる幅にし、本文やモバイルナビゲーション、Toast と重ならない。
  Popover は画面内で左上へ開き、長い文言は2行まで自然に折り返す。設定 section は1列。
- 768px: サイドバーや toolbar と視覚的に競合しない右下に置く。Popover は件数を2列相当で読める
  幅を取り、主要操作を覆い続けない。
...
通常表示と概要表示を撮る。UI 画像は次を含める。

- 総数未確定の実行中: 割合が出ず、indeterminate と文言で待っていることが分かる。
- 総数確定の実行中: 右下の短い表示、概要、設定 section が同じ割合と件数を示す。
...
- **視覚的階層**: 右下表示は補助状態として控えめで、失敗だけが必要な強さを持つ。設定 section は
