...
6. 取り込み中にライブラリの閲覧、検索、filter、フォルダ移動を行い、動画を開いて再生できることを確認する。再生画面でも同じ indicator が右下で更新され、player control や閉じる × を覆わないことを確認する。
...
1. 360px、768px、1280px と高さの狭い viewport で、通常画面と再生画面の表示、hover/focus 概要を撮影する。右端と下端から安全な余白があり、画面外へはみ出さず、閉じる ×、player control、属性情報を隠さないことを確認する。
