...
| 1 | 判定: `sdd-target.sh` で対象 feature を一度だけ確定し、その `spec.md` の `Parent Issue` のラベルで workflow を決める。確定した feature と workflow で state、guard を各1回実行する | `guard.go = true`、または `reason = open-pr` | Parent Issue の欠落・重複・不正・取得失敗は `remote-unavailable` と同じ扱い。`open-pr` は手順 1.5 でレビュー有無を確認する。open PR が無い branch は、closed 未マージ PR の `head.sha` が guard の `open_heads` の SHA と一致するときだけ、その SHA への `--force-with-lease` で削除して guard をやり直す（S7）。一致しない・PR が無い branch は削除せず報告して終了。`phase-retry-limit` / `hop-limit` なら Issue を立てて終了。それ以外は理由を書いて終了 |
...
| 5 | コミット・push・PR 作成・ラベル付与（下記） | PR の URL が得られ、ラベル `sdd` が付いている | PR 作成の呼び出しが失敗したら、まず `head=` 指定で open PR を 1 件検索し（サーバー側では成功している場合がある）、あればそれを使う。無ければ push した SHA に対する `--force-with-lease` で自分の段階 branch を remote から消し、理由を報告して終了する（残すと後続セッションが作成中と区別できず待ち続ける）。ラベル付与の失敗は cloud の組み込み GitHub ツールで 1 回再試行。それでも失敗なら PR 本文の先頭に「ラベル未付与」と書いて終了 |
