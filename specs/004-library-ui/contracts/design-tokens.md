...
列数は画面幅ではなく**器の幅**で決まる。一覧の器には `max-w-6xl`（1152px）が掛かって
いるので、左右の余白 `1rem` を引いた器の幅は 1120px で頭打ちになる。そこに届くのは画面幅
1392px（= 1152 + サイドバーの 240）からで、それを超えて広げても列は増えない。
...
（`--tile-gap` = `1rem`、左右の余白 = `1rem`。上の値は
[005 の quickstart.md](../../005-ui-refinement/quickstart.md) S2 の 4 つの幅を Chromium で
実測したものである。）

**器の幅は 005 で変わった。** 幅 640px 以上ではコンテンツ領域から骨格のサイドバー
（240px）が引かれるためで、格子の式も 3 つの `--size-tile-*` も変えていない。列数は
[005 の contracts/layout.md](../../005-ui-refinement/contracts/layout.md) 1. が式から先に
決めてあり、上の実測はその表と一致した。導出の前提（サイドバー 240px・器の上限 1152px・
余白 16px・gap 16px）が実装と一致していることの確認でもある。

器に上限を置いたまま列数を増やさないのは、帯・見出し・一覧が同じ幅の柱にそろっている
ためである。一覧だけを画面幅いっぱいに広げると、帯の中の操作と一覧の左端がずれる。
2560px で 12 列や 15 列に広げる案は、その柱を崩す変更になるので本機能では採らない。
