...
| probe未完了・失敗、尺なし、映像streamなし | `409` | `conflict` |
...
scaleとresolution renditionは追加しない。
...
| --- | --- |
| H.264 video | copy |
| その他のvideo | H.264 (`libx264`, `yuv420p`, `preset=veryfast`, `crf=23`) |
| AAC audio | copy |
| その他のaudio | AAC stereo 192kbps |
| audioなし | audioを出力しない |

### Normalize mode

`playable=true`と判定済みの動画がdirect media error後にこの経路へ来た場合に使う。映像を上表のH.264
設定、音声があればAAC設定へ変換し、取り込み時判定が見落としたprofile/pixel format/browser差を
copyし直さない。

## FFmpeg and request lifecycle

- input-side seekを使い、映像copy時は直前keyframeから音声との同期を保つ
