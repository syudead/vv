...
- **シェル**: 上部の `TopBar`（`web/src/shell/TopBar.tsx`）は ☰・ロゴ・取り込みの
  ...
  （`TopBarPortal`）。ナビゲーションと設定への入口は `Sidebar` にある。
...
- **カード**（`web/src/videoList/VideoCard.tsx`）: サムネイル、その右下の再生時間、
  下端の再生進捗、題名を描く。ライブラリの格子表示では題名の下にその動画のタグの行を
  足し、タグの無い動画では足さない（specs/014-video-tags/ui-design.md「Library card」）。
...
- **リスト表示**（ライブラリのみ）: 1 行に題名・視聴済み・長さ・画質・大きさ・追加日時を
  並べる。
- **選択**（ライブラリのみ）: 格子表示のチェックはポイントまたはフォーカスしたときに
  ...
  件数・タグを一括で付ける・外す操作・「すべて選択」・選択を解除するボタン（Esc でも
  解除できる）が並ぶ。「すべて選択」は、読み込んでいないページを含む今の
  条件の全件を選ぶ。sm 未満ではバーが 2 段になり、タグの操作が下の段に回る
  （specs/014-video-tags/ui-design.md「Selection bar」）。選択が
  `POST /api/video-tags` の上限（20,000件、全部か無しか。
  specs/014-video-tags/contracts/tags-api.md §4）を超えるときは、タグの一括操作が
...
- **構成要素**: プレイヤー・題名・タグ・属性情報（ファイルの場所を含む）・関連動画を置く。
  シェルと上部バーは無く、閉じる操作は × と Esc である。戻り先は画面を開く前の
...
- **video.js の部品になっているのは操作バーの部品**: 再生速度・10 秒送り・現在時刻/長さ・
