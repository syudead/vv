...
- **ツールバー**: 検索・絞り込み・表示形式（ライブラリのみ）・表示倍率・並び順を持つ。
  ...
  ライブラリではツールバーの下の行に、フォルダ画面では一群の見出しに出る。
...
- **リスト表示**（ライブラリのみ）: 1 行に題名・視聴済み・長さ・画質・大きさ・追加日時を
  並べる。
- **選択**（ライブラリのみ）: チェックはポイントまたはフォーカスしたときに表示され、
  hover できない端末では常に表示される。1 件でも選択されたら全件のチェックを表示し、
  画面下部に固定の選択バーを出す。選択バーには件数・タグを一括で付ける・外す操作・
  「すべて選択」・「選択解除」が並ぶ。「すべて選択」は、読み込んでいないページを含む今の
