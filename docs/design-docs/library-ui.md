...
- **構成要素**: プレイヤー・題名・タグ・公開の切り替え・属性情報（ファイルの場所を含む）・
  関連動画を置く。シェルと上部バーは無く、閉じる操作は × と Esc である。戻り先は画面を
  開く前の一覧で、関連動画や「次を再生」で移っても最初の一覧のまま引き継ぐ。タグは題名の
  すぐ下に置き、題名と1つのまとまりとして扱う（詳細は
  [specs/014-video-tags/ui-design.md「Video page tags」](../../specs/014-video-tags/ui-design.md#video-page-tags)）。
  公開の切り替え（`role="switch"`）はそのまとまりの直下、区切り線の上に置く。ゲストでは
  タグ・公開の切り替え・ファイルの場所・LAST PLAYED を出さず、題名の下は区切り線と属性の
  一列だけになる（[specs/016-single-account-auth/ui-design.md「Visibility toggle」「Guest degradation」](../../specs/016-single-account-auth/ui-design.md#visibility-toggle)）。
- **幅による出し分けは CSS**（4 と同じ）: `lg` 以上は右に関連動画の列、それ未満は縦に
  積む。× は幅ごとに 1 つずつ DOM にあり、`hidden`／`lg:hidden` で見せる方が決まる。
- **属性は区切り線の下の 1 列**: 英語の小さいラベルと値の組を `dl` で並べる。縦線で
  区切った 1 行になるのは収まる `xl` からで、それ未満は線の無い格子である。折り返す幅で
  縦線を使うと、行頭に線と余白が残るからである。
