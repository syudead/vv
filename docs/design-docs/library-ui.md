...
- **リスト表示**（ライブラリのみ）: 1 行に題名・画像・視聴済み・長さ・画質・大きさ・追加日時を
  並べる。題名を先に置いて左端を揃え、画像は「長さ」の手前に右寄せで置く。グループの行は
  題名と画像の列を1つにまとめ、題名の下に本数、右にメンバーのサムネイル（最大8枚）を
  横一列に並べたフォルダの絵柄を題名の側へ広げ（入らない分は省く）、合計の長さと大きさを
  出し、画質は空にする
