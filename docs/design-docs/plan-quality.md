...
### P-8: 凍結済み仕様を置き換えるなら、置き換える先を名指しする

`specs/001-008` は凍結済みで、作られた当時の機能を記述したまま動かさない。この feature が
そこに書かれた振る舞いを変える場合、`## 既存仕様への影響` に、ファイルと置き換える要求の
識別子（`FR-003`、`SC-007` など）を書く。凍結済みファイル自身は編集しない。

置き換えが無いなら、この節は置かない（P-7）。「既存仕様と矛盾しない」と書くために節を
作らない。

この節が必要なのは、親Issueからは判定できないからである。何を置き換えるかは既存の
`specs/` を読んで初めて分かるので、要求の側ではなく Plan が引き受ける。
