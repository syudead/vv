...
- **影響範囲**: [仕様品質の規則](../product-specs/spec-quality.md) Q-6 の承認ゲート
- **内容**: 仕様作成者とは独立した reviewer の承認を必須にしているが、CI や
  スクリプトでの強制は無い。判定対象が自然言語の仕様であり、機械検査にすると
  「検査しやすい離散条件だけへ変換する」という #43 が批判する振る舞いを
  ゲート自体が再現しかねないため、当面は PR レビュー（と PR テンプレートの
  「仕様の変更」節）で実施する。承認を経ない仕様変更 PR が出ても機械的には止まらない。
- **当面の対処**: [レビュー手順](../how-to/spec-quality-review.md)どおりに回し、
  承認の記録を PR に残す。
- **見直しの契機**: 仕様の体裁（要求原文の引用欄・対応表の有無など）を壊さず検査できる
