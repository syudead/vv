...
| 仕様 | 完了 |
| 計画・設計 | 完了 |
| タスク分解 | 完了（旧タスク成果物は後に廃止） |
| 実装 | 完了（43 タスクすべて） |
| 受け入れ検証（S1〜S10） | 完了。S1〜S10 すべて期待どおり。制約は下記の1点のみ |
...
- **TanStack Router／Query の導入は Phase 1 へ送る。** Phase 0 の画面は
  1つで、ルーティングもサーバー状態のキャッシュも必要ない。
- **憲章（`.specify/memory/constitution.md`）は雛形のまま。** 計画の
  ゲートは `ARCHITECTURE.md` と技術選定文書の判断基準から導出した。憲章を作る場合は
  この計画と矛盾しないか確認すること。
- **`web/embed.go` を置いて埋め込みの宣言だけを `web/` に持たせた。**
...
- **Web の書式は Prettier を入れた。** 契約は `make fmt` を
...
- **受け入れ検証のうち Docker と CI に依るものを残した。** 実装した環境に
...
- **受け入れ検証はコンテナ経路まで完了した。** 当初この環境では Docker が
...
- **補助的に使った確認手段。** `docker compose config`（デーモン不要で
