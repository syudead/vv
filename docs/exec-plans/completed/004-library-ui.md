...
| 計画・設計成果物 | 完了 |
| タスク分解 | 完了。T001〜T051 |
| Phase 1: Setup（単体テストの実行基盤） | 完了。T001〜T004 |
| Phase 2: Foundational（トークンと回帰の網） | 完了。T005〜T013 |
| Phase 3: US1（一覧が「自分のライブラリ」に見える） | 完了。T014〜T023。S3 は人の確認待ち |
| Phase 4: US2（再生画面が同じ規則で整う） | 完了。T024〜T031。S8・S5 は人の確認待ち |
| Phase 5: US3（表示の仕方を自分で選べる） | 完了。T032〜T037。S6 は人の確認待ち |
| Phase 6: US4（小さな画面でも破綻しない） | 完了。T038〜T041。S4 の機械で測れる部分（4 幅の横スクロール・当たり判定・列数）は済み、実機の目視は人の確認待ち |
| Phase 7: Polish | 完了。T042〜T051 |
| 受け入れ検証（S1・S2） | 完了。`make check` が緑。下表参照 |
| 受け入れ検証（S4・S7・S8 の一部・S10） | 完了。見本データ + Chromium で実行。下表参照 |
| 受け入れ検証（S0・S3・S5・S6・S9 の実機分） | **未実施**。この環境に `ffmpeg` と Docker の実行基盤が無い。保守者に残る（下表参照） |

## 受け入れ検証の結果（Phase 7 / T046〜T050）

実行環境は Claude Code on the web のセッション（Linux コンテナ）で、
