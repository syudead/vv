...
**残りの構図の確認（[quickstart.md](../../../specs/005-ui-refinement/quickstart.md) S2）は未実施である。**
幅 640 / 639 の境界、グリッドをスクロールしたときに固定領域が動かないこと、そして
密度を変えたあとに戻した項目がヘッダーの裏へ隠れないこと（004 の S6-2 / T014）は、
この回では確かめていない。最後の 1 つは画面を埋めるだけの件数（確認用は 6 本しか無い）が
要る。US2 で中身が入ってから S2・S3・S8 をまとめて実行する。
