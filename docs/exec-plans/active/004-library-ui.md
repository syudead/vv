...
### `ScanStatus` の `onFinished` を「見届けた取り込み」に限った（T030 の付随）

`GET /api/scans/current` は実行中のものが無ければ**最後に終わったもの**を返すため、
`ScanStatus` は最初の巡回で `done` を読んだだけで `onFinished` を呼んでいた。これは
一覧を開くたびに起きるので、T030 の復元は毎回その場で捨てられていた（控えを消し、
`reload()` が項目を空にする）。実行中を見たとき、または利用者が「取り込む」を押した
ときにだけ知らせるようにした。

この印は**部品より長く生きるモジュール変数**にしてある。部品の中に持つと、一覧で
実行中を見たあと再生画面へ移り、その間に取り込みが終わった場合を知らせそこなう
（戻ってきた一覧が控えのまま古い件数を出し続ける）。タブを読み込み直せば消える寿命で、
それは控え（`web/src/api/listSnapshot.ts`）と同じである。

印を降ろす／立てる条件はレビューで 3 回直している（前に立てて失敗時に残る、成功時しか
見張り直さない、など）。あわせて「取り込みを始められなかった」ことを状態の取得の失敗とは
別に持つようにした ── 同じ入れ物だと、押した直後の巡回が成功しただけで消え、押した利用者
には始まったように見える。**この部分は素直さを欠いており、控えにスキャン ID を持たせて
`LibraryPage` 側で判断する設計（Devin の当初案）のほうが状態の置き場所としては良い。**
PR #27 のレビューにその旨を残してある。

T016 は `ScanStatus` の振る舞いを変えないと定めているが、
[contracts/screen-states.md](../../../specs/004-library-ui/contracts/screen-states.md) 4.
が求めているのは「終わったら一覧を読み直す」ことであって、開くたびに読み直すことでは
ない。副産物として、一覧を開くたびに起きていた 1 ページ目の二重取得も無くなった。
`web/src/components/ScanStatus.test.tsx` が 3 つの場合を固定している。
