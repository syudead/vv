...
| S3〜S8 の実機確認 | 同 S3〜S8 |
| S4 の実機での目視（4 幅の見え方） | 同 S4。列数・横スクロール・当たり判定は T040 で Chromium 実測済み（[contracts/design-tokens.md](../../../specs/004-library-ui/contracts/design-tokens.md) 3. の表を実測値に更新済み）。残るのは実機の見え方の確認 |
