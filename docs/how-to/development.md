...
configuration changes run `task check`; the Docker build additionally requires
a pull request to `main` or a push to `main`, and browser E2E runs only on a
push to `main`, that is, after a pull request is merged.
