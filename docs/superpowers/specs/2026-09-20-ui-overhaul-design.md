...
## 再改訂（2026-09-20、独自トーンへ）

Stash のトレースになりすぎたため、構造（上部ナビ・フィルタ帯・箱型カード・
件数行・リスト表示）は残し、見た目を vv 独自に改めた。

- 配色: 暖色寄りのチャコール（`#16181d` / `#1e2129` / `#2a2e38`）にミントの
  アクセント（`#2fd0b4`）。青灰・青アクセントは使わない。
- 書体: Inter Variable + Noto Sans JP Variable（セルフホスト）。角丸 4 / 6 / 10px。
- ナビバーは地と同色 + 下境界線（濃い帯にしない）。ブランドはミントの点 + 「vv」。
- カードは 10px 角丸、ホバーで浮き上がり + サムネイル微ズーム。画質・尺は
  影付き文字ではなく半透明の小片（画質はアクセント色）。
- タブの下線はアクセント色。

## 再々改訂（2026-09-20、操作配置の整理）

- 左サイドバーを復帰し、ライブラリの入口と設定を置く。設定はサイドバー末尾に固定する。
- 検索・絞り込み・並び順・表示形式・ズームはトップバーへ集約する。
- 取り込み操作はトップバー右端に残す。
- モバイルではサイドバーをドロワーにし、閉じている間はフォーカス順と支援技術から除外する。
