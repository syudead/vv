...
目標そのものを呼ぶことで、手元と CI の判定を一致させている。`task test-e2e` と
Docker イメージのビルドは、main 向けの PR と main への push でだけ実行する。
main 向けでも、PR の変更が文書（`*.md`、`docs/`、`specs/`、`.agents/`、`.claude/`、
`renovate.json`）だけのときは `task test-e2e` を飛ばす。
