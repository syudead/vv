...
func TestRemoveOrphanPreviewsKeepsReferencedPair(t *testing.T) {
	dir := t.TempDir()
	for _, key := range []string{"keep", "remove"} {
		path := PreviewPath(dir, key)
		...
		if err := os.WriteFile(PreviewManifestPath(dir, key), []byte("manifest"), 0o600); err != nil {
			t.Fatal(err)
		}
	}
	removed, err := RemoveOrphanPreviews(dir, map[string]struct{}{"keep": {}})
	if err != nil {
		t.Fatal(err)
	}
	if removed != 2 {
		t.Fatalf("removed = %d, want 2 files", removed)
	}
	if _, err := os.Stat(PreviewPath(dir, "keep")); err != nil {
		t.Fatalf("referenced preview was removed: %v", err)
	}
	if _, err := os.Stat(PreviewPath(dir, "remove")); !errors.Is(err, os.ErrNotExist) {
		t.Fatalf("orphan preview remains: %v", err)
	}
}

func TestRemoveAbandonedPreviewTempsRespectsCutoff(t *testing.T) {
	dir := t.TempDir()
	prefix := filepath.Join(dir, "preview", "ab")
	old := filepath.Join(prefix, ".preview-old")
	active := filepath.Join(prefix, ".preview-active")
	for _, path := range []string{old, active} {
		if err := os.MkdirAll(path, 0o755); err != nil {
			t.Fatal(err)
		}
		if err := os.WriteFile(filepath.Join(path, "preview.mp4"), []byte("partial"), 0o600); err != nil {
			t.Fatal(err)
		}
	}
	now := time.Now()
	...
	removed, err := RemoveAbandonedPreviewTemps(dir, now.Add(-10*time.Minute))
	if err != nil {
		t.Fatal(err)
	}
	if removed != 1 {
	...
	if _, err := os.Stat(old); !errors.Is(err, os.ErrNotExist) {
		t.Fatalf("old temporary directory remains: %v", err)
	}
	if _, err := os.Stat(active); err != nil {
		t.Fatalf("active temporary directory was removed: %v", err)
	}
	removed, err = RemoveAbandonedPreviewTemps(dir, time.Now().Add(time.Second))
	if err != nil || removed != 1 {
		t.Fatalf("startup cleanup = %d, %v", removed, err)
	}
}
