...
func TestJobLifecycle(t *testing.T) {
	...
	if job.Kind != domain.JobProbe || job.VideoID != videoID {
		t.Errorf("取り出したジョブ = %+v", job)
	}

	// 専有したので、2 度目の取り出しでは何も返らない。
	if _, err := db.Ingest().ClaimJob(ctx, domain.JobProbe); !errors.Is(err, domain.ErrNoJob) {
		t.Errorf("同じジョブが二重に取り出せた: %v", err)
	}

	if err := db.Ingest().CompleteJob(ctx, job.ID); err != nil {
		t.Fatal(err)
	}
...
func TestClaimJobOnEmptyQueue(t *testing.T) {
	...
	if _, err := db.Ingest().ClaimJob(context.Background(), domain.JobProbe); !errors.Is(err, domain.ErrNoJob) {
		t.Errorf("err = %v, want domain.ErrNoJob", err)
	}
}
...
func TestFailJobRetriesThenGivesUp(t *testing.T) {
	...
	for attempt := 1; attempt <= domain.MaxJobAttempts; attempt++ {
		job, err := db.Ingest().ClaimJob(ctx, domain.JobProbe)
		if err != nil {
		...
		if err := db.Ingest().FailJob(ctx, job.ID, "ffprobe が失敗しました"); err != nil {
			t.Fatal(err)
		}

		want := "queued"
		if attempt == domain.MaxJobAttempts {
			want = "failed"
		}
	...
	if domain.MaxJobAttempts != 3 {
		t.Errorf("MaxJobAttempts = %d, want 3", domain.MaxJobAttempts)
	}
}
...
func TestFailJobRecordsReason(t *testing.T) {
	...
	if err := db.Ingest().FailJob(ctx, job.ID, "壊れた JSON"); err != nil {
		t.Fatal(err)
	}
...
func TestEnqueueJobIsIdempotentWhilePending(t *testing.T) {
	...
	// 種類が違えば別のジョブである。
	if err := db.Ingest().EnqueueJob(ctx, domain.JobThumbnail, videoID); err != nil {
		t.Fatal(err)
	}
...
func TestEnqueueJobRetriesAfterFailure(t *testing.T) {
	...
	for i := 0; i < domain.MaxJobAttempts; i++ {
		job, err := db.Ingest().ClaimJob(ctx, domain.JobProbe)
		if err != nil {
			t.Fatal(err)
		}
		if err := db.Ingest().FailJob(ctx, job.ID, "失敗"); err != nil {
			t.Fatal(err)
		}
	}
...
func TestEnsureJobDoesNotReviveTerminalFailure(t *testing.T) {
	...
	if err := db.Ingest().EnsureJob(ctx, domain.JobProbe, videoID); err != nil {
		t.Fatal(err)
	}
	if _, err := db.Ingest().ClaimJob(ctx, domain.JobProbe); !errors.Is(err, domain.ErrNoJob) {
...
func TestEnsureJobRequeuesLegacyFailureOfPendingVideo(t *testing.T) {
	...
	for range domain.MaxJobAttempts {
		...
		// 旧版と同じく、行にだけ失敗を記録する。
		if err := db.Ingest().FailJob(ctx, job.ID, "unreadable location"); err != nil {
			t.Fatal(err)
		}
	}
...
func TestRequeueRunningJobs(t *testing.T) {
	...
	restored, err := db.Ingest().RequeueRunningJobs(ctx)
	if err != nil {
		t.Fatal(err)
	}
	if restored != 1 {
...
func TestClaimJobTriesEveryLocationBeforeConsumingAnotherAttempt(t *testing.T) {
	...
	for i := range 4 * domain.MaxJobAttempts {
		job, err := db.Ingest().ClaimJob(ctx, domain.JobProbe)
		if err != nil {
			t.Fatal(err)
		}
		...
		wantState := "queued"
		if i == 4*domain.MaxJobAttempts-1 {
			wantState = "failed"
		}
	...
	if _, err := db.Ingest().ClaimJob(ctx, domain.JobProbe); !errors.Is(err, domain.ErrNoJob) {
		t.Fatalf("ClaimJob after final cycle error = %v, want domain.ErrNoJob", err)
	}
}
...
func TestClaimJobWaitsForMigratedLocationToBeRegistered(t *testing.T) {
	...
	ctx := context.Background()
	root := t.TempDir()
	video, err := db.ScanIndex().UpsertVideo(ctx, sampleFile(filepath.Join(root, "movie.mp4"), "movie", "migrated", 1, 0))
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := db.Ingest().ClaimJob(ctx, domain.JobThumbnail); !errors.Is(err, domain.ErrNoJob) {
		t.Fatalf("unregistered ClaimJob error = %v, want domain.ErrNoJob", err)
	}
	...
	if got := recorder.take(); !slices.Contains(got, domain.JobThumbnail) {
		t.Fatalf("フォルダ登録の知らせ = %v, want thumbnail を含む", got)
	}
	job, err := db.Ingest().ClaimJob(ctx, domain.JobThumbnail)
	if err != nil {
		t.Fatal(err)
	}
...
func TestClaimedJobBecomesStaleWhenLocationIsAdded(t *testing.T) {
	...
	written, err := db.Ingest().ApplyProbeForJob(ctx, job, domain.Probe{VideoCodec: "h264", AudioCodec: "aac"}, domain.Playability{Playable: true})
	if err != nil {
		t.Fatal(err)
	}
	if written {
...
func TestPreviewStateUsesContentIdentityForSuccessAndClaimIdentityForFailure(t *testing.T) {
	...
	written, err := db.Ingest().SetPreviewStateForContent(ctx, job, domain.PreviewStateDone)
	if err != nil || !written {
		t.Fatalf("content-key completion = %v, %v", written, err)
	}
	if err := db.Ingest().SetPreviewState(ctx, videoID, domain.PreviewStatePending); err != nil {
		t.Fatal(err)
	}
	written, err = db.Ingest().SetPreviewStateForJob(ctx, job, domain.PreviewStateFailed)
	if err != nil {
		t.Fatal(err)
	}
	if written {
...
func TestPreviewRunningJobIsRequeuedAfterRestart(t *testing.T) {
	...
	claimed, err := db.Ingest().ClaimJob(ctx, domain.JobPreview)
	if err != nil {
		t.Fatal(err)
	}
	if claimed.Kind != domain.JobPreview {
	...
	retried, err := db.Ingest().ClaimJob(ctx, domain.JobPreview)
	if err != nil {
		t.Fatal(err)
	}
	if retried.ID != claimed.ID || retried.Kind != domain.JobPreview {
...
func TestPreviewSourceRejectsReassignedClaimedPathWithOriginalContentRemaining(t *testing.T) {
	...
	sourceCurrent, err := db.Ingest().PreviewSourceCurrent(ctx, job)
	if err != nil {
		t.Fatal(err)
	}
	if sourceCurrent {
...
func TestCompletePreviewAtomicallyFinishesAssetAndJobAfterCancellation(t *testing.T) {
	...
	cancel()
	applied, err := db.Ingest().CompletePreviewForContent(context.WithoutCancel(cancelled), job)
	if err != nil || !applied {
...
func TestClaimedJobBecomesStaleWhenLowerIDLocationIsReassigned(t *testing.T) {
	...
	if err := db.Ingest().EnqueueJob(ctx, domain.JobProbe, target.ID); err != nil {
		t.Fatal(err)
	}
	job, err := db.Ingest().ClaimJob(ctx, domain.JobProbe)
	if err != nil {
	...
	written, err := db.Ingest().ApplyProbeForJob(ctx, job, domain.Probe{VideoCodec: "h264"}, domain.Playability{Playable: true})
	if err != nil {
		t.Fatal(err)
	}
	if written {
...
func TestClaimJobTakesOnlyTheRequestedKind(t *testing.T) {
	...
	if _, err := db.Ingest().ClaimJob(ctx, domain.JobProbe); !errors.Is(err, domain.ErrNoJob) {
		t.Fatalf("別の段階の ClaimJob error = %v, want domain.ErrNoJob", err)
	}
	job, err := db.Ingest().ClaimJob(ctx, domain.JobThumbnail)
	if err != nil {
		t.Fatal(err)
	}
	if job.Kind != domain.JobThumbnail {
...
func TestJobsQueuedNotification(t *testing.T) {
	...
	if err := db.Ingest().EnsureJob(ctx, domain.JobThumbnail, videoID); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := db.Ingest().RequeueRunningJobs(ctx); err != nil {
		t.Fatal(err)
	}
	if got := recorder.take(); len(got) != 0 {
...
func TestProcessingCountsRemainingWorkPerStage(t *testing.T) {
	...
	for _, job := range []struct {
		...
		{domain.JobProbe, videoID}, {domain.JobProbe, other.ID}, {domain.JobThumbnail, videoID},
		{domain.JobPreview, other.ID}, {domain.JobProbe, outside.ID},
	} {
	...
	probeDone(t, db, videoID)
	if _, err := db.Ingest().ClaimJob(ctx, domain.JobThumbnail); err != nil {
		t.Fatal(err)
	}
	// 終わった仕事は数えない。
	preview, err := db.Ingest().ClaimJob(ctx, domain.JobPreview)
	if err != nil {
		t.Fatal(err)
	}
	if err := db.Ingest().CompleteJob(ctx, preview.ID); err != nil {
		t.Fatal(err)
	}
...
func TestClaimThumbnailWaitsForProbe(t *testing.T) {
	...
	if _, err := db.Ingest().ClaimJob(ctx, domain.JobThumbnail); !errors.Is(err, domain.ErrNoJob) {
		t.Fatalf("解析前の ClaimJob error = %v, want domain.ErrNoJob", err)
	}
...
func TestRequeueMissingPreview(t *testing.T) {
	...
	ctx := context.Background()
	probeDone(t, db, videoID)
	if err := db.Ingest().SetPreviewState(ctx, videoID, domain.PreviewStateDone); err != nil {
		t.Fatal(err)
	}
	recorder := &queuedRecorder{}
	db.PublishTo(recorder)

	if requeued, err := db.Ingest().RequeueMissingPreview(ctx, videoID, "other-key"); err != nil || requeued {
		t.Fatalf("内容の違う要求 = %v, %v, want false", requeued, err)
	}
	requeued, err := db.Ingest().RequeueMissingPreview(ctx, videoID, "key-a")
	if err != nil || !requeued {
	...
	if _, err := db.Ingest().ClaimJob(ctx, domain.JobPreview); !errors.Is(err, domain.ErrNoJob) {
